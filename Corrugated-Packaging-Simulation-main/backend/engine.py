...
class CorrugatedSimulation:
    def __init__(self, machines_df: pd.DataFrame, jobs_df: pd.DataFrame, routings_df: pd.DataFrame, forklift_count=2):
        self.health_check(machines_df, jobs_df, routings_df)
        ...
        self.buffers = {}
        self.machine_stats = {} 
        self.machine_current_state = {} 
        
        for _, row in machines_df.iterrows():
            m_id = row['Machine_ID']
            count = int(row['Count'])
            buffer_cap = int(row['Input_Buffer_Capacity'])
            ...
            self.machine_stats[m_id] = {
                'working_time': 0, 'setup_time': 0, 'blocked_time': 0, 
                'starved_time': 0, 'down_time': 0, 'completed_operations': 0,
                'buffer_wip_area': 0, # length * duration for time-weighted average
        ...
        self.forklift = simpy.Resource(self.env, capacity=forklift_count)
    ...
    def update_buffer_wip(self, m_id):
        """Track the time-weighted average of the buffer length."""
        now = self.env.now
        prev = self.machine_stats[m_id]['last_buffer_update']
        duration = now - prev
        if duration > 0:
            current_len = len(self.buffers[m_id].items)
            self.machine_stats[m_id]['buffer_wip_area'] += current_len * duration
        self.machine_stats[m_id]['last_buffer_update'] = now

    def health_check(self, m: pd.DataFrame, j: pd.DataFrame, r: pd.DataFrame):
        """Ensure all required columns are present and data types are valid."""
        req = {
            "Machines": (m, ["Machine_ID", "Count"]),
            "Jobs": (j, ["Job_Type", "Batch_Size", "Target_Demand"]),
            "Routings": (r, ["Job_Type", "Sequence_Order", "Machine_ID", "Setup_Time_Base", "Process_Time_Per_Unit"])
        }
        for sheet, (df, cols) in req.items():
            missing = [c for c in cols if c not in df.columns]
            if missing:
                raise ValueError(f"CRITICAL: Sheet '{sheet}' is missing required columns: {missing}. Please check 'What-If' JSON casing.")
        self.wip_timeline = [] 
    ...
    def setup_factory(self):
        """Setup secondary processes like machine breakdowns."""
        self.change_wip(0)
        ...
        for _, row in self.machines_df.iterrows():
            ...
            # Start breakdown processes for each physical machine unit
        ...
        for idx, step in routing.iterrows():
            ...
            with self.machines[m_id].request(priority=1) as machine_req:
                ...
                if next_step_idx < len(routing):
                    ...
                    self.update_buffer_wip(next_m_id)
    ...
    def job_source(self, job_type):
        ...
        # Fallback for missing arrival intervals in config
        if 'Interarrival_Min' in self.jobs_df.columns:
            arr_min = self.jobs_df.loc[self.jobs_df['Job_Type'] == job_type, 'Interarrival_Min'].iloc[0]
        else:
            arr_min = 0.5
            
        if 'Interarrival_Max' in self.jobs_df.columns:
        ...
        for i in range(batches_needed):
            ...
            self.update_buffer_wip(first_machine)
    ...
    def run(self, until_time=None):
        ...
        # Final WIP Area Update & Average Calculation
        final_time = self.env.now
        for m_id in self.machine_stats:
            self.update_buffer_wip(m_id) 
            area = self.machine_stats[m_id]['buffer_wip_area']
            self.machine_stats[m_id]['average_wip'] = round(area / final_time, 3) if final_time > 0 else 0
